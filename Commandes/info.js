...
exports.run = (client, message, args) => {
  ...
  .setDescription("Information pour " + n.name)
  .setThumbnail(`${client.user.avatarURL}`)
  .setAuthor(message.author.username + "#" + message.author.discriminator, `${message.author.avatarURL}`)
  .setColor(randomColor())
  .addField("Nombre total d'utilisateurs desservant",
    client.users.size, true)
  .addField("Uptime",
    msToTime(), true)
  .addField("Prêt à",
    `${human.date('m-d-y | h:i:s', client.readyAt)} ${amPm()}`, true)
  .addField("Utilisation de la mémoire",
    `Using ${bytes(process.memoryUsage().rss)}`, true)
  .addField("Serveurs total des guildes",
    `${client.guilds.size}`, true)
  .addField("Version",
    `${n.version}`)
  .setFooter("coder par marceau#4082")
  .setTimestamp()
